// src/index.js — V2 Market Maker (Websocket + Trailing Stop + Stale Order Detection)
...
function checkStop(mid) {
  ...
  if (loss >= STOP.MAX_LOSS) { log(`🛑 MAX LOSS: cost ₦${state.lastBuyFillPrice.toFixed(2)} now ₦${mid.toFixed(2)} loss ₦${loss.toFixed(2)}`); return true; }
...
async function executeTrade(book) {
  ...
  try {
    ...
    if (state.buyOrderId && state.buyOrderId !== 'COOLDOWN') {
      const gap = book.bestBid - state.buyOrderPrice;
      if (gap > 0.50) {
    ...
    // === SIZING — 35% to 70% ===
    const sr = Math.min(book.spread/config.SPREAD_SCALE, 1.0);
    const cap = Math.max(0.35, 0.70*sr);
    let bv = Math.floor((state.ngnBalance*cap/bp)*100)/100;
    let sv = Math.floor(state.usdtBalance*cap*100)/100;
    ...
    if (inv > config.REBALANCE_THRESHOLD) { sv=Math.floor(state.usdtBalance*0.80*100)/100; sv=Math.min(sv,config.MAX_ORDER_USDT); }
    else if (inv < 1-config.REBALANCE_THRESHOLD) { bv=Math.floor((state.ngnBalance*0.80/bp)*100)/100; bv=Math.min(bv,config.MAX_ORDER_USDT); }

    // Refresh
    if (state.buyOrderId&&state.buyOrderId!=='COOLDOWN'&&state.buyOrderPrice<bp-0.005) {
      try{await rest.cancelOrder(state.buyOrderId)}catch(e){} state.buyOrderId=null;
    }
    if (state.sellOrderId&&state.sellOrderId!=='COOLDOWN'&&state.sellOrderPrice>sp+0.005) {
      const ok = !state.lastBuyFillPrice || sp > state.lastBuyFillPrice+0.05;
...
async function main() {
  ...
  log('🚀 V2 — Websocket + Trailing Stop + Stale Detection');
  ...
  http.createServer((req,res)=>{res.writeHead(200);res.end(JSON.stringify({status:'running',v:'v2.1',spread:lastBook?lastBook.spread.toFixed(2):'N/A',pnl:state.dailyPnl.toFixed(2),rot:state.dailyRotations,cost:state.lastBuyFillPrice}));}).listen(PORT,()=>log(`Port ${PORT}`));
}
